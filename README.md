...
> 支持范围：Netflix 网页端、英文源字幕、简体中文目标字幕、Chrome 111+ 或兼容的 Chromium 系浏览器。

## 工作方式

SubTwin 采用“字幕来源由用户显式选择”的路由：

| 当前状态 | 行为 | 外部翻译调用 |
| --- | --- | --- |
| 选择 Netflix 原生双语，且同时提供官方英文和简体中文 | 解析两条官方轨道，按时间对齐并显示双语 | **0** |
| 字幕目录仍在发现中，尚未确认完整 | 保留 Netflix 原生字幕，不把“暂未看到中文”当作“没有中文” | **0** |
| 选择 Netflix 原生双语，但只有英文 | 保留 Netflix 原生英文字幕 | **0** |
| 选择 Google 翻译 | 逐条使用 Google 翻译；即使有官方中文也按所选来源翻译 | 只调用 Google |
| 选择 DeepSeek | 使用上下文批次翻译并校验结果；即使有官方中文也按所选来源翻译 | 只调用 DeepSeek |
| DeepSeek 未配置 Key、翻译失败或离线 | 保留可用英文/原生字幕并显示状态 | 不切换到 Google |

三个来源之间不会自动切换：选择哪一个，就严格使用哪一个。Google 与 DeepSeek 之间也没有自动降级。切换来源、切换剧集、关闭扩展或播放器重挂载时，旧一代任务会被取消；迟到结果不能继续渲染或写入缓存。
...
Netflix 播放页会额外注入一个独立 Shadow DOM 控制界面。默认只显示可拖拽的红色“双”悬浮按钮，点击后展开侧边控制台；可直接选择 Netflix 原生双语、Google 翻译或 DeepSeek 翻译，在当前页暂停/继续 SubTwin，或全局启用/关闭扩展，并实时调整中英文显示、上下顺序、字体、字号、粗细、颜色、阴影、行距、单行长度、垂直位置和背景透明度。单行长度可在 50%–100% 间调整，用于减少长英文与中文的换行。控制台打开时还可直接上下拖动字幕，移动过程实时预览、松开后自动保存；关闭控制台后字幕层恢复鼠标穿透，不影响 Netflix 操作。控制台还会显示当前影片、播放状态、Netflix 可访问的音轨/字幕轨、官方中文字幕目录、当前字幕来源和错误状态。修改先作用于当前字幕层，再经严格的 content-to-background 消息自动保存；API Key 和模型不会进入 Netflix 页面消息。
...
- Chrome 111+，或兼容该版本 MV3 能力的 Chromium 浏览器
...
5. 打开扩展选项页，选择翻译方式；也可在 Netflix 页点击红色“双”悬浮按钮直接调整字幕样式。
